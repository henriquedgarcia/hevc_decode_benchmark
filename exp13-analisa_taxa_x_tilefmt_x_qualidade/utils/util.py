...
class Atribs:
    ...
    def mp4_folder(self):
        ...
        self._mp4_folder = f'{self.project}{self.sl}{self.mp4_base}'
...
class VideoParams(Atribs):
    def __init__(self, config, yuv='yuv', hevc_base='hevc', mp4_base='mp4', segment_base='segment',
                 dectime_base='dectime', project='ffmpeg', encoder='ffmpeg', factor='crf'):
        super().__init__()
        # Initial actions
        self.sl = check_system()['sl']
        self.config = config
...
def encode(video):
    global_params = '-hide_banner -y -nostats'
    param_in = f'-i {video.yuv}{video.sl}{video.name}.mp4'
    ...
    for y in range(0, video.height, video.tile_h):
        for x in range(0, video.width, video.tile_w):
            tile_count += 1
            filter_params = f'-vf "setdar=2,scale={video.scale},crop=w={video.tile_w}:h={video.tile_h}:x={x}:y={y}" -map 0:0'
            video.mp4_video = f'{video.mp4_folder}{video.sl}{video.basename}_tile{tile_count}'

            command = f'{video.program} {global_params} {param_in} {param_out} {filter_params} {video.mp4_video}.mp4'

            proc = subprocess.run(command, shell=True, encoding='utf-8', stdout=subprocess.PIPE, stderr=subprocess.STDOUT, universal_newlines=True)

            output = '\n'.join(proc.stdout.strip().splitlines()[-9:])  # Somente as 10 ultimas linhas

            with open(video.mp4_video + '.txt', 'w', encoding='utf-8') as f:
                f.write(output)
...
def run(command, video_name, ext, overwrite=False, log_mode='w'):
    if os.path.isfile(f'{video_name}.{ext}') and not overwrite:
        ...
        if log_mode in 'none':
            ...
            with open(video_name + '.txt', log_mode, encoding='utf-8') as f:
