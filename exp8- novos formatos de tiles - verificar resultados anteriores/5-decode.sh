...
for tile in ${tile_list}; do	# Para cada tile
    ...
    for chunk in $(seq -f "%03g" 1 60); do
        ...
        for rate in ${rate_list}; do	# para cada taxa
            std_name="dash/"${video}"_"${size}"_"${fps}"_"${tile}"_rate"${rate}"/segments/"${video}"_tile"${t}"_track"$((t+1))"_"${chunk}".mp4"
            out_name="dectime/"${video}"_"${size}"_"${fps}"_"${tile}"_rate"${rate}"_tile"${t}"_"${chunk}".log"
